...
public final class FileIOUtils {
    ...
    public static List<String> readFile2List(final File file, final int st, final int end, final String charsetName) {
        if (!isFileExists(file)) return new ArrayList<>();
        if (st > end) return new ArrayList<>();
        try (BufferedReader reader = isSpace(charsetName) ?
     ...
     * @return 字符串
     */
    public static String readFile2StringDelSpecial(final String filePath) {
        return readFile2StringDelSpecial(getFileByPath(filePath), null);
    }
     ...
     * @return 字符串
     */
    public static String readFile2StringDelSpecial (final File file, final String charsetName) {
        if (!isFileExists(file)) return null;
        ...
        try {
            ...
            String line;
            while ((line = reader.readLine()) != null) {
                String text = Utils.delSpecialCharacters(line);
                if (TextUtils.isEmpty(text)){
                    continue;
                }
                sb.append(text).append(LINE_SEP);
            }
            // delete the last line separator
            return sb.delete(sb.length() - LINE_SEP.length(), sb.length()).toString();
        } catch (Exception e) {
//            e.printStackTrace();
    ...
    public static byte[] readFile2BytesByStream(final File file) {
        if (!isFileExists(file)) return new byte[0];
