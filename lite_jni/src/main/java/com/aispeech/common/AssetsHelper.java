...
public class AssetsHelper {
    ...
    private synchronized static void initMD5Map(Context context) {
        ...
        copyDestFile(inputStream, cacheFile);
        FileUtils.createOrExistsFile(cacheFile); // 若不存在assets md5 表，创建空文件占位
     ...
     * 更新拷贝的资源映射
     */
    public synchronized static void updateMapFile(Context context) {
    ...
    public static synchronized int copyResource(Context context, String resName, boolean isMD5, String resMd5sumName) {
        return copyResource(context, resName, isMD5, resMd5sumName, null);
    }
    ...
    public static synchronized int copyResource(Context context, String resName, boolean isMD5, String resMd5sumName, String destPath) {
        ...
        try {
            ...
            // 如果需要拷贝压缩文件 尝试删除上一次解压的文件目录
            if (destFile.getName().endsWith(".zip")) {
                boolean deleteDir = FileUtils.deleteDir(destFile.getParentFile());
                Log.i(TAG, "delete old zip dir :" + destFile.getParentFile().getAbsolutePath()
                        + " ,result:" + deleteDir);
            }
    ...
    private static boolean checkMD5(InputStream inputStream, File file) {
        try {
            return realGetAssetsMD5(inputStream).equals(bytesToHex(FileUtils.getFileMD5(file)));
    ...
    private static boolean copyDestFile(InputStream inputStream, File destFile) {
        if (inputStream == null) {
            Log.w(TAG, "copyDestFile inputStream = null");
            return false;
        }
    ...
    private static boolean unZip(File zipfileName, File destDir) {
        Log.i(TAG, "start unzip" + zipfileName.getName());
...
//                Log.d(TAG, "unzip: " + entry.getName());
                ...
                File destItemFile = new File(destDir,
                        entry.getName());
                FileUtils.createOrExistsFile(destItemFile);
    ...
    private static void makeMD5(String resName, File destFile) {
        ...
        String md5 = bytesToHex(FileUtils.getFileMD5(destFile));
