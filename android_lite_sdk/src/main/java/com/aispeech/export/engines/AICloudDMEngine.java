...
/**
 * 云端对话引擎
 *@deprecated 废弃，参考{@link com.aispeech.export.engines2.AICloudDMEngine}
 * @author hehr
 */
@Deprecated
public class AICloudDMEngine {
    private static final String TAG = "AICloudDMEngine";
    private com.aispeech.export.engines2.AICloudDMEngine cloudDMEngine;


    private AICloudDMEngine() {
        cloudDMEngine = com.aispeech.export.engines2.AICloudDMEngine.createInstance();
    }
    ...
    public void init(AICloudDMConfig config, AICloudDMListener listener) {
        if (cloudDMEngine != null) {
            cloudDMEngine.init(config, listener);
        }
    }
     ...
     *
     * @param data 音频数据流
     * @see AICloudDMConfig.Builder#setUseCustomFeed(boolean)
     */
    public void feedData(byte[] data) {
        if (cloudDMEngine != null) {
            cloudDMEngine.feedData(data);
        }
    }
    ...
    public void feedback2PRIVCloud(String topic, String data) {
        if (cloudDMEngine != null) {
            cloudDMEngine.feedback2PRIVCloud(topic, data);
        }
    }
    ...
    public void updateVocabs(AIUpdateListener listener, Vocab... vocabs) {
        if (cloudDMEngine != null) {
            cloudDMEngine.updateVocabs(listener, vocabs);
        }
    }
    ...
    public void updateProductContext(AIUpdateListener listener, ProductContext context) {
        if (cloudDMEngine != null) {
            cloudDMEngine.updateProductContext(listener, context);
        }
    }
    ...
    public void updateSkillContext(AIUpdateListener listener, SkillContext context) {
        if (cloudDMEngine != null) {
            cloudDMEngine.updateSkillContext(listener, context);
        }
    }
    ...
    public void triggerIntent(SkillIntent intent) {
        if (cloudDMEngine != null) {
            cloudDMEngine.triggerIntent(intent);
        }
    }
    ...
    public void triggerIntent(String skill, String task, String intent, String slots) {
        if (cloudDMEngine != null) {
            cloudDMEngine.triggerIntent(skill, task, intent, slots);
        }
    }
    ...
    public void async(MultiModal multiModal) {
        if (cloudDMEngine != null) {
            cloudDMEngine.async(multiModal);
        }
    }
    ...
    public void setExcludeDispatch(AIUpdateListener listener, String[] skillList, String option) {
        if (cloudDMEngine != null) {
            cloudDMEngine.setExcludeDispatch(listener, skillList, option);
        }
    }
    ...
    public void setRejectAfterDispatch(AIUpdateListener listener, String[] skillList, boolean enable) {
        if (cloudDMEngine != null) {
            cloudDMEngine.setRejectAfterDispatch(listener, skillList, enable);
        }
    }
    ...
    public boolean isConnected() {
        if (cloudDMEngine != null) {
            return cloudDMEngine.isConnected();
        }
        return false;
    }
}
