...
public abstract class NluInterceptor extends AbstractCostInterceptor {
    ...
    public JSONObject handleIntercept(String layer, String flowType, JSONObject custom) {
        String nluResult = customIntercept(layer, flowType, custom, custom == null ? null : custom.optString(getName()));
        try {
            if (custom != null) {
                custom.put(getName(), nluResult);
            }
